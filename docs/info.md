...
1. **Baseline**: RO off vs on (`ui[7]`), log the divided-RO frequency per tap on
   `uio[5]` with a scope or counter.
2. **Static calibration (frequency mode)**: set `ui[6]=1`, power the breakout
   ...
   through it.
3. **Droop (period mode)**: set `ui[6]=0`, arm periodic bursts (`ui[1:0]=11`),
